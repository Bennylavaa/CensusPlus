...
--
---------------------------------------------------------------------------------
local CensusPlus_VERSION = "5.0.0"; 			-- version
...
local MAX_WHO_RESULTS = 30;						-- Maximum number of who results the server will return (lowered for testing)
...
function CensusPlus_Msg(msg)
	...
	-- Only show messages if verbose mode is enabled
	if( CensusPlus_PerCharInfo and CensusPlus_PerCharInfo["Verbose"] == true ) then
...
local g_LastWhoSendTime = 0;					-- Last time we sent a /who command
...
function CensusPlus_GetFactionRaces(faction)
	-- Modified to return ALL races regardless of faction to show cross-faction data
	local ret = {CENSUSPlus_DWARF, CENSUSPlus_GNOME, CENSUSPlus_HUMAN, CENSUSPlus_NIGHTELF,
		CENSUSPlus_ORC, CENSUSPlus_TAUREN, CENSUSPlus_TROLL, CENSUSPlus_UNDEAD};
	return ret;
end
...
local function GetRaceClasses(race)
	-- Modified to return ALL classes for every race, ignoring game restrictions
	local ret = {CENSUSPlus_WARRIOR, CENSUSPlus_PALADIN, CENSUSPlus_HUNTER, CENSUSPlus_ROGUE, 
...
function CensusPlus_OnLoad()
	...
	--
	CensusPlusText:SetText("Epoch Census v"..CensusPlus_VERSION .. " " .. g_CensusPlusLocale );
...
function CP_ProcessWhoEvent(query, ...)
	local numWhoResults, totalCount = GetNumWhoResults();
	
	-- Debug logging to see what values we're getting
	CensusPlus_Msg("DEBUG: GetNumWhoResults returned: numWhoResults=" .. (numWhoResults or "nil") .. ", totalCount=" .. (totalCount or "nil"));
	...
	-- Check if we should split - trying both values
	CensusPlus_Msg("DEBUG: Checking split - numWhoResults(" .. numWhoResults .. ") > MAX_WHO_RESULTS(" .. MAX_WHO_RESULTS .. ") = " .. tostring(numWhoResults > MAX_WHO_RESULTS));
	
	if (numWhoResults > MAX_WHO_RESULTS) then
		--
		-- Who list is overflowed, split the query to make the return smaller
		--
		CensusPlus_Msg("DEBUG: SPLITTING QUERY! Results exceeded threshold.");
		...
		if (minLevel ~= maxLevel) then
			CensusPlus_Msg("DEBUG: Splitting by level - " .. minLevel .. "-" .. maxLevel);
...
--		CensusPlus_DetermineServerDate();
	...
	elseif( string.lower(param) == "rank" or string.lower(param) == "ranking" ) then
		local rankStart, rankEnd, totalPlayers = CensusPlus_GetPlayerLevelRanking();
		if rankStart and rankEnd and totalPlayers and totalPlayers > 0 then
			local playerLevel = UnitLevel("player");
			if rankStart == rankEnd then
				CensusPlus_Msg("You are level " .. playerLevel .. " - Rank: " .. rankStart .. " of " .. totalPlayers .. " players");
			else
				CensusPlus_Msg("You are level " .. playerLevel .. " - Rank: " .. rankStart .. "-" .. rankEnd .. " of " .. totalPlayers .. " players");
			end
		else
			CensusPlus_Msg("No ranking data available. Run a census first to gather player data.");
		end
	...
	elseif( command ~= nil and string.lower(command) == "test" ) then
		...
		-- Just show the main window when no parameters are given
		CensusPlus:Show();
	end
end
...
--    CensusPlus_Msg("  /CensusPlus locale  Bring up the locale selection dialog - (WARNING -- CHANGING YOUR LOCALE WILL PURGE YOUR DATABASE)");
    ...
    CensusPlus_Msg("  /CensusPlus rank Show your level ranking among all players");
...
function CensusPlus_Verbose()
    if( CensusPlus_PerCharInfo["Verbose"] == true ) then
        ChatFrame1:AddMessage("Census+: Verbose Mode : OFF", 1.0, 1.0, 0.5);
        CensusPlus_PerCharInfo["Verbose"] = false;
	else
        ChatFrame1:AddMessage("Census+: Verbose Mode : ON", 1.0, 1.0, 0.5);
...
function CensusPlus_OnUpdate()
	...
	if (g_IsCensusPlusInProgress == true and g_CensusPlusPaused == false and g_CensusPlusManuallyPaused == false ) then
		...
		local whoSendDelta = now - g_LastWhoSendTime;
		if (not g_WaitingForWhoUpdate or delta > CensusPlus_UPDATEDELAY) then
			if (whoSendDelta < CensusPlus_UPDATEDELAY and g_LastWhoSendTime > 0) then
				return;
			end
...
function CensusPlus_ProcessWhoResults()
	...
	CensusPlus_Msg("DEBUG: ProcessWhoResults - numWhoResults=" .. (numWhoResults or "nil") .. ", totalCount=" .. (totalCount or "nil"));
...
function CensusPlus_UpdateView()
	...
	if rankStart and rankEnd and totalPlayers and totalPlayers > 0 then
		...
		local rankingText = "";
		if rankStart == rankEnd then
			rankingText = "Your Level " .. playerLevel .. " Rank: #" .. rankStart .. " of " .. totalPlayers .. " players";
		else
			rankingText = "Your Level " .. playerLevel .. " Rank: #" .. rankStart .. "-" .. rankEnd .. " of " .. totalPlayers .. " players";
		end
		
		-- Set the ranking text to the dedicated field
		if CensusPlusPlayerRanking then
			CensusPlusPlayerRanking:SetText(rankingText);
		end
	else
		-- Clear the ranking text if no data
		if CensusPlusPlayerRanking then
			CensusPlusPlayerRanking:SetText("");
		end
	end
	...
	-- Hide the 10th class slot (was for Death Knights)
	local legend10 = getglobal("CensusPlusClassLegend10");
	local bar10 = getglobal("CensusPlusClassBar10");
	if legend10 then
		legend10:Hide();
	end
	if bar10 then
...
----------------------------------------------------------------------------------
--
-- Calculate player's level ranking
--
---------------------------------------------------------------------------------
function CensusPlus_GetPlayerLevelRanking()
	if( g_CensusPlusLocale == "N/A" ) then
		return nil, nil, nil;
	end
	...
	if not playerName or not playerLevel or not realmName or not factionGroup then
		return nil, nil, nil;
	end
	
	local totalPlayers = 0;
	local playersAbove = 0;
	local playersAtSameLevel = 0;
	
	-- Walk through the database and count players
	local realmDatabase = CensusPlus_Database["Servers"][realmName];
	if realmDatabase then
		local factionDatabase = realmDatabase[factionGroup];
		if factionDatabase then
			for race, raceData in pairs(factionDatabase) do
				if type(raceData) == "table" then
					for class, classData in pairs(raceData) do
						if type(classData) == "table" then
							for name, charData in pairs(classData) do
								if type(charData) == "table" and charData[1] then
									local level = tonumber(charData[1]);
									if level then
										totalPlayers = totalPlayers + 1;
										if level > playerLevel then
											playersAbove = playersAbove + 1;
										elseif level == playerLevel then
											playersAtSameLevel = playersAtSameLevel + 1;
										end
									end
								end
							end
	...
	-- Calculate ranking range
	local rankingStart = playersAbove + 1;
	local rankingEnd = playersAbove + playersAtSameLevel;
	
	return rankingStart, rankingEnd, totalPlayers;
end
